class MsgController < Merb::Controller
  ...
  def get_folder_data(folder,offset)
		case folder
			when 'inbox', 'sent'
				...
				txt = "Msg.all_msgs = #{@msgs.to_json}; serverTime=parseFloat("+ (("%10.5f" % Time.now.to_f).to_f * 1000).to_s + "); Msg.msgs_in_#{folder}=#{@total}; "
	...
	def validate_user
	  @user = User.find(:first, :select => [:id, :username],  :conditions => ["member_login_key=?", cookies[:upload_id]])
	end
end