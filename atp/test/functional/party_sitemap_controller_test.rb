...
require 'party_sitemap_controller'

# Re-raise errors caught by the controller.
class PartySitemapController; def rescue_action(e) raise e end; end

class PartySitemapControllerTest < Test::Unit::TestCase

  fixtures :parties

  def setup
    @controller = PartySitemapController.new
  ...
  def test_should_render_sitemap
    get :index
    assert_template "parties/party_sitemap"
    assert_response :success
    assert assigns(:host)
    assert assigns(:parties)
  end
  ...
  def test_should_route_to_sitemap
    assert_routing "party/sitemap", :controller => "party_sitemap", :action => 'index'
  end
