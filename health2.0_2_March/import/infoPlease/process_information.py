...
def CompanyInsertCheckByPost(companyName, private_notes, url, enabled, description):
    ...
    info["commit"]= "Create"
    url = "http://"+host+":"+port+"/companies.xml"
    print url
    try:
...
def PersonnelInsertCheck(first_name, last_name, title,  companyID):
    ...
    info["commit"]= "Create"
    url = "http://"+host+":"+port+"/personnels.xml"
    try:
        data = urllib.urlencode(info)
        req = urllib2.Request(url,data)
        response = urllib2.urlopen(req)
...
def EmailInsertCheck(email,type, personID):
    ...
    info["commit"]= "Create"
    url = "http://"+host+":"+port+"/email_addresses.xml"
    try:
        data = urllib.urlencode(info)
        req = urllib2.Request(url,data)
        response = urllib2.urlopen(req)
...
def PhoneInsertCheck(number,type, personID):
    ...
    info["commit"]= "Create"
    url = "http://"+host+":"+port+"/phone_numbers.xml"
    try:
        data = urllib.urlencode(info)
        req = urllib2.Request(url,data)
        response = urllib2.urlopen(req)
...
for datalist in f:
	...
	product += datalist[16].replace(r'<[^>]*>', '')
	product += datalist[18].replace(r'<[^>]*>', '')
	product += datalist[19].replace(r'<[^>]*>', '')
#
#	conference_session = ""
#	stealth_mode = datalist[15]
	financing_history = datalist[17]
#	product += datalist[14].replace(r'<[^>]*>', '')
