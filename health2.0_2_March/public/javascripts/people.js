...
$.ajax({
   ...
   datatype: "xml",
   complete: function(){ global_array[0] = 1;},
...
function people_form_new(key){
var fi = new Array();
fi["id"] = companyID;
fi["key"]=key;
   $.ajax({
   type: "GET",
   url: urlProcess("/personnels/add_people"),
   data: returnStringArray(fi),
   datatype: "xml",
   success: function(xml){
   ModalPopups.Alert('1','Add People',xml);
...
var fi = new Array();
   ...
   datatype: "xml",
   success: function(xml){
   ModalPopups.Alert('1','Edit People',xml);
