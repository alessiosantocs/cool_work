class Admin::ScheduleController < ApplicationController
  ...
  def index
    @state = State.find(:all, :order => :name)
  ...
  def copy_to_locations
    serviced_zips = @selected_state.serviced_zips
    @locations = Location.find(:all, :conditions => ["target_type = ? AND target_id in(?) ", 'ServicedZip',serviced_zips])
    schedule = Schedule.for(@selected_location, params[:date].to_date)
  ...
  def set_location(location=params[:location],state_location=params[:state_location])
    @selected_state = state_location.nil? ? (session[:selected_state].nil? ? State.find(:first) : State.find(session[:selected_state])) : State.find(state_location)
    session[:selected_state] = @selected_state.id if !@selected_state.nil?
    if !state_location.blank?
     @selected_location = (@selected_state.blank? or @selected_state.serviced_zips.blank?) ? ServicedZip.find(session[:selected_location]) : @selected_state.serviced_zips.first
    else
