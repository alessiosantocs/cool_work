...
class Customer < ActiveRecord::Base
  ...
  def registered_invitees_detail
    User.find(:all,
          :conditions => ["invitations.recipient_email is not null and invitations.sender_id = ? and invitations.send_reminder_email = ?", self.account,false], 
          :joins => "left join invitations on (users.email = invitations.recipient_email)" )
  end
...
#   end
  ...
  def is_renew
    ...
    recurring_orders.each do |recurring_order|
      last_order = recurring_order.last_order
  ...
  def self.signup_invitee
    customers = self.all
    registered_invitees = []
      customers.each do |customer|
        registered_invitees<<customer.registered_invitees_detail
      end
      registered_invitees.flatten.each do |registered_invitee|
        if registered_invitee.created_at.to_date + 1.month <= Time.now.to_date && registered_invitee.account.orders.blank?
          Notifier.deliver_send_reminder_mail_for_order(registered_invitee)
          invitation = Invitation.find_by_recipient_email(registered_invitee.email)
          invitation.update_attribute(:send_reminder_email,true)
        end
      end
  end
    ...
    def give_fresh_cash_for_eco_points
      if self.points >=5000 
        ...
        self.fresh_cash = self.fresh_cash.to_i+500
        self.save
      end
      if self.points >=2000 and self.points<5000 
        self.points = self.points - 2000
        self.fresh_cash = self.fresh_cash.to_i+200
        self.save
      end
      if self.points >=1000 and self.points<2000 
        self.points = self.points - 1000
        self.fresh_cash = self.fresh_cash.to_i+100
        self.save
      end
      if self.points >=500 and self.points<1000 
        self.points = self.points - 500
        self.fresh_cash = self.fresh_cash.to_i+50
        self.save
      end
  end
